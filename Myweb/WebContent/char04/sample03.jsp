...
<html>
	...
	<script type="text/javascript">	
		$(document).ready(function() {
			/*p em선택자는 p태그 아래의 모든 em태그*/
			$('p em').css('background-color', 'yellow');
			/*p > em선택자는 p태그 직계의 모든 em태그*/
			$('p > em').css('color', 'red');
			...
			$('div em').css('background-color', 'aqua');
			/*p > em선택자는 p태그 직계의 모든 em태그*/
			$('div > em').css('color', 'pink');
			...
			$('div em').each(function() {
				...
				var message = '';
				message = '첫번째 요소:' + $('tr:first').text() + '<br>';
				message += '마지막 요소:' + $('tr:last').text() + '<br>';
 				
				$('#myarea').html(message);
				
				$('.mytest span:first').css('color','red');
				$('.mytest span:last').css('color','pink');
